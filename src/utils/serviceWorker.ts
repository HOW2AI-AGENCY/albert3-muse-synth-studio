...
class ServiceWorkerManager {
            ...
            /**
             * Полная очистка хранилища приложения (LocalStorage, SessionStorage, Caches)
             * Используется при ошибках квоты
             */
            async clearAppStorage(): Promise<void> {
              try {
                logger.warn('Запущена полная очистка хранилища', 'SW Manager');
                ...
                // 3. Clear Service Worker Cache via Message
                await this.clearAudioCache();

                logger.info('Хранилище успешно очищено', 'SW Manager');
                window.location.reload(); // Reload to reset state
              } catch (error) {
                logger.error('Ошибка при очистке хранилища', error instanceof Error ? error : new Error(String(error)), 'SW Manager');
              }
            }
...
export const serviceWorkerManager = new ServiceWorkerManager();
        ...
        /**
         * Экспорт функции для полной очистки хранилища
         */
        export const clearAppStorage = async (): Promise<void> => {
          return serviceWorkerManager.clearAppStorage();
        };
