...
const WavConvertMenuItem: React.FC<{
  ...
  return <DropdownMenuItem onClick={handleConvert} disabled={isConverting && convertingTrackId === trackId}>
...
const gradients = ["from-purple-500/10 to-pink-500/10", "from-blue-500/10 to-cyan-500/10", "from-green-500/10 to-emerald-500/10", "from-orange-500/10 to-red-500/10"];
...
const GenerationProgress: React.FC<{
  ...
  return <div className="absolute inset-0 bg-black/70 backdrop-blur-sm flex flex-col items-center justify-center p-3 text-white z-10 text-center transition-opacity duration-300">
...
const TrackCardComponent = ({
  ...
  useEffect(() => {
    const checkStems = async () => {
      ...
      const {
        data
      } = await supabase.from('track_stems').select('id').eq('track_id', track.id).limit(1);
  ...
  return <motion.div ref={cardRef} variants={fadeInUp} initial="initial" animate="animate" whileHover={{
    y: -4
  }} whileTap={{
    scale: 0.98
      ...
      <div className="relative aspect-square bg-gradient-to-br from-gray-800 to-gray-900">
        ...
        {track.status === 'completed' && <div className="absolute top-2 right-2 z-10">
            <TrackVariantSelector trackId={track.id} currentVersionIndex={selectedVersionIndex} onVersionChange={handleVersionChange} />
          </div>}

        {displayedVersion.cover_url || track.cover_url ? <LazyImage src={displayedVersion.cover_url || track.cover_url || '/placeholder.svg'} alt={`Обложка трека ${displayedVersion.title || track.title}`} placeholder="/placeholder.svg" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" wrapperClassName="w-full h-full" /> : <div className={cn("w-full h-full flex items-center justify-center bg-gradient-to-br", gradient)}>
            <Music className="w-8 h-8 text-primary/50" />
          </div>}

        {track.status === 'completed' && <div className={cn("absolute inset-0 bg-black/40 flex items-center justify-center transition-opacity duration-300", isHovered || isCurrentTrack && isPlaying ? "opacity-100" : "opacity-0 group-focus-within:opacity-100")}>
            <Button variant="secondary" size="icon" onClick={handlePlayClick} disabled={playButtonDisabled} className="rounded-full w-10 h-10 shadow-lg hover:scale-110 transition-transform" aria-label={isCurrentTrack && isPlaying ? "Приостановить" : "Воспроизвести"}>
      ...
      <CardContent className="p-2 flex-1 flex flex-col">
        ...
        {(track.status === 'processing' || track.status === 'pending') && <div className="mb-2">
            <TrackProgressBar progress={track.progress_percent || 0} status={track.status} createdAt={track.created_at} />
          </div>}
          ...
          <div className="flex items-center">
            ...
            {track.status === 'completed' && <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="w-6 h-6" onClick={e => e.stopPropagation()} aria-label="Опции">
...
const MemoizedTrackCard = memo(TrackCardComponent, (prevProps, nextProps) => {
  return prevProps.track.id === nextProps.track.id && prevProps.track.status === nextProps.track.status && prevProps.track.like_count === nextProps.track.like_count && prevProps.track.is_public === nextProps.track.is_public && prevProps.track.audio_url === nextProps.track.audio_url && prevProps.track.cover_url === nextProps.track.cover_url && prevProps.track.duration === nextProps.track.duration && prevProps.track.metadata?.reference_audio_url === nextProps.track.metadata?.reference_audio_url && prevProps.track.progress_percent === nextProps.track.progress_percent && prevProps.onClick === nextProps.onClick;
});
