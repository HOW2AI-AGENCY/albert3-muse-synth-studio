...
export const TagsCarousel = ({ onTagClick, disabled = false }: TagsCarouselProps) => {
  return (
    <div className="space-y-1">
      ...
      <ScrollArea className="w-full whitespace-nowrap rounded-md border border-border/40 bg-secondary/20 p-1.5">
        <div className="flex gap-1.5">
          {INSPIRATION_TAGS.map((tag) => (
            <Button
              ...
              variant="outline"
              size="sm"
              className="shrink-0 gap-1.5 h-7 text-xs font-normal hover:bg-primary/10 hover:text-primary hover:border-primary/30 transition-all"
