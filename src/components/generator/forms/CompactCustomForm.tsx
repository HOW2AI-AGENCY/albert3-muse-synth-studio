...
const AudioDescriber = lazy(() => import('@/components/audio/AudioDescriber').then(m => ({
  default: m.AudioDescriber
})));
...
export const CompactCustomForm = memo(({
  ...
  const handleQuickTagAdd = useCallback((tag: string) => {
    ...
    if (!existingTags.includes(tag)) {
      ...
      logger.info('Quick tag added', 'CompactCustomForm', {
        tag
      });
    }
  ...
  return <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pb-20">
        ...
        <div className="space-y-1.5 p-2">
          <div className="flex items-center justify-between">
            <PromptCharacterCounter currentLength={debouncedPrompt.length} maxLength={MAX_PROMPT_LENGTH} />
            {onOpenHistory && <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" onClick={onOpenHistory} disabled={isGenerating} className="h-6 w-6">
          ...
          <div className="relative">
            <Textarea placeholder="Опишите стиль, жанр, настроение..." value={debouncedPrompt} onChange={e => {
            if (e.target.value.length <= MAX_PROMPT_LENGTH) {
              onDebouncedPromptChange(e.target.value);
            }
          }} disabled={isGenerating} className={cn("pr-10 resize-y min-h-[80px] max-h-[300px]", isMobile ? "text-base" : "text-sm")} maxLength={MAX_PROMPT_LENGTH} />
            {onBoostPrompt && debouncedPrompt.trim() && <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" className="absolute right-1 top-2 h-6 w-6 text-primary hover:text-primary hover:bg-primary/10" onClick={onBoostPrompt} disabled={isBoosting || isGenerating}>
        ...
        <div className="space-y-1 p-2">
          ...
          <Input id="custom-title" type="text" placeholder="Авто-генерация если пусто" value={params.title} onChange={e => onParamChange('title', e.target.value)} className={cn(isMobile ? "h-10 text-base" : "h-8 text-sm")} disabled={isGenerating} maxLength={80} />
        </div>

        {/* Selected Resources Info */}
        {(params.referenceFileName || params.personaId || params.inspoProjectName) && <div className="p-2 space-y-1 border border-accent/40 rounded-lg bg-accent/5">
            {params.referenceFileName && <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {params.personaId && <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <User className="h-3 w-3" />
                <span className="flex-1">Persona: Active</span>
                <Button variant="ghost" size="icon" className="h-4 w-4 hover:text-destructive" onClick={() => onParamChange('personaId', null)}>
                  ×
                </Button>
              </div>}
            {params.inspoProjectName && <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <Collapsible defaultOpen={!!debouncedLyrics}>
          <CollapsibleTrigger className="flex items-center justify-between w-full p-2 hover:bg-accent/5 rounded-md transition-colors group">
            <div className="flex items-center gap-2 text-sm font-medium">
              ...
              <span>Lyrics</span>
              {lyricsLineCount > 0 && <Badge variant="secondary" className="h-5 text-[10px] px-1.5">
          ...
          <CollapsibleContent className="pt-2">
            <LyricsInput value={debouncedLyrics} onChange={onDebouncedLyricsChange} onGenerateLyrics={onOpenLyricsDialog} isGenerating={isGenerating} />
          </CollapsibleContent>
        </Collapsible>
        ...
        <Collapsible defaultOpen={tagsCount > 0}>
          <CollapsibleTrigger className="flex items-center justify-between w-full p-2 hover:bg-accent/5 rounded-md transition-colors group">
            <div className="flex items-center gap-2 text-sm font-medium">
              <ChevronDown className="h-4 w-4 transition-transform group-data-[state=open]:rotate-180" />
              <span>Styles</span>
              {tagsCount > 0 && <Badge variant="secondary" className="h-5 text-[10px] px-1.5">
                  {tagsCount}
                </Badge>}
            </div>
          </CollapsibleTrigger>
          <CollapsibleContent className="pt-2 space-y-2">
            {/* AI Recommendations */}
            {debouncedPrompt.length >= 10 && <StyleRecommendationsInline prompt={debouncedPrompt} currentTags={params.tags.split(',').map(t => t.trim()).filter(Boolean)} lyrics={params.lyrics} onApplyTags={handleApplyTags} onAdvancedPromptGenerated={handleAdvancedPromptGenerated} />}

            <StyleTagsInput tags={params.tags} negativeTags={params.negativeTags} onTagsChange={tags => onParamChange('tags', tags)} onNegativeTagsChange={tags => onParamChange('negativeTags', tags)} isGenerating={isGenerating} />
            
            {/* Quick Style Pills */}
            <div className="flex flex-wrap gap-1.5">
              {['creepy', 'ambient', 'acid techno', 'synthwave', 'lofi'].map(tag => <Button key={tag} variant="outline" size="sm" onClick={() => handleQuickTagAdd(tag)} className="h-6 px-2 text-[10px] gap-1" disabled={isGenerating}>
            ...
            {params.referenceAudioUrl && <Suspense fallback={<div className="text-xs text-muted-foreground">Загрузка анализатора...</div>}>
                <AudioDescriber audioUrl={params.referenceAudioUrl} onDescriptionGenerated={description => onParamChange('prompt', description)} />
              </Suspense>}
          </CollapsibleContent>
        </Collapsible>
        ...
        <Collapsible>
          ...
          <CollapsibleContent className="pt-2 space-y-3">
            {/* Advanced Controls */}
            <AdvancedControls provider={params.provider as MusicProvider} vocalGender={params.vocalGender} audioWeight={params.audioWeight} styleWeight={params.styleWeight} lyricsWeight={params.lyricsWeight} weirdness={params.weirdnessConstraint} hasReferenceAudio={!!params.referenceFileName} hasLyrics={!!debouncedLyrics.trim()} onVocalGenderChange={value => onParamChange('vocalGender', value)} onAudioWeightChange={value => onParamChange('audioWeight', value)} onStyleWeightChange={value => onParamChange('styleWeight', value)} onLyricsWeightChange={value => onParamChange('lyricsWeight', value)} onWeirdnessChange={value => onParamChange('weirdnessConstraint', value)} isGenerating={isGenerating} />
          </CollapsibleContent>
        </Collapsible>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 border-t border-border/20 bg-background/95 backdrop-blur-sm">
        <div className="p-3 flex items-center gap-2">
          {/* Create Button */}
          <Button onClick={onGenerate} disabled={isGenerating || !debouncedPrompt.trim() && !debouncedLyrics.trim()} size="lg" className={cn("w-full gap-2 font-semibold bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70", isMobile ? "h-12 text-base" : "h-10 text-sm")}>
