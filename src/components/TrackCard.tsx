...
export const TrackCard = ({ 
  ...
  const handleShare = async () => {
    if (navigator.share && (track as any).is_public) {
      try {
  ...
  const getStatusBadge = () => {
    switch (track.status) {
      case 'completed':
        return <Badge variant="default" className="animate-pulse-glow bg-green-500 text-white">Готов</Badge>;
      case 'processing':
              ...
              <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                {(track as any).profiles?.username && (
                  <div className="flex items-center gap-1">
                    ...
                    <span>{(track as any).profiles?.username}</span>
                  </div>
                )}
                <div className="flex items-center gap-1">
