...
- **Приоритет**: HIGH
- **Оценка**: 10 часов
- **Статус**: В прогрессе (частично выполнено)
...
- **Задачи**:
  ...
  - [x] Настроить сбор рендер-метрик через React Profiler (App.tsx).
  - [x] Создать KPI-модуль для метрик производительности (latency, error rate, memory).
  - [x] Инструментировать ключевые API-вызовы (Supabase Edge) таймерами и Sentry.
  - [ ] Создать дашборд (Grafana/Looker) для FCP, LCP, CLS, TTFB.
  - [ ] Настроить мониторинг размера бандла и времени ответа API.
  - [x] Документировать систему (docs/performance/KPI_MONITORING.md).
- **Критерии приемки**:
  - Ключевые метрики доступны в режиме quasi real-time.
  - Алерты срабатывают на отклонения от порогов.
  - Отчётность синхронизирована с дашбордом и KPI-модулем.
