...
const mainHandler = async (req: Request) => {
  ...
  try {
    ...
    // ✅ FIX: Игнорируем 'text' callback, но обрабатываем 'first' для быстрого UI апдейта
    if (callbackType === 'text') {
      logger.info('⏭️ Skipping text callback, waiting for audio', 'suno-callback', {
        callbackType,
        taskId
      });
      return new Response(JSON.stringify({ ok: true, message: 'text_callback_acknowledged' }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (successTask && tasks.length > 0) {
      ...
      // Always update with the title from the callback, as it's the most recent
      if (sanitizedTitle && sanitizedTitle !== 'Generated Track' && sanitizedTitle !== track.title) {
        updateData.title = sanitizedTitle;
        logger.info('Updating track title from callback', 'suno-callback', {
          old: track.title,
          new: sanitizedTitle 
        });
      }
