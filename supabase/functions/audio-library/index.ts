...
Deno.serve(async (req: any) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (error) {
        logger.error('Error creating audio entry', error instanceof Error ? error : new Error(String(error)), 'audio-library');
        return new Response(
          JSON.stringify({ error: error.message }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
    ...
    logger.error('Error in audio-library function', error instanceof Error ? error : new Error(String(error)), 'audio-library');
    return new Response(
