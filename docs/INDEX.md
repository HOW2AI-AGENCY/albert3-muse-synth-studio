# 📚 Documentation Index

Центральный индекс всей документации проекта Albert3 Muse Synth Studio.

## 🚀 Quick Start
- [README](../README.md) — Обзор проекта
- [Development Guide](./DEVELOPMENT.md) — Локальная разработка
- [Contributing Guide](../CONTRIBUTING.md) — Вклад в проект
 - [Archive Policy](./ARCHIVE_POLICY.md) — Политика хранения и структура архива

## 🏗️ Архитектура
- [Architecture Overview](./ARCHITECTURE.md) — Полная архитектура
- [Data Flow Diagrams](./diagrams/data-flow.md) — Потоки данных
- [Database Schema](./diagrams/database-schema.md) — Схема БД

## 🔒 Protected Components
- [Protected Files List](./PROTECTED_FILES.md) — Файлы под защитой
- [CODEOWNERS](../.github/CODEOWNERS) — Владельцы компонентов

## 🤖 Automation
- [GitHub Automation](./GITHUB_AUTOMATION.md) — CI/CD, Dependabot
 - [Audit Reports](./audit/README.md) — Индекс аудитов и отчётов
 - [Archive Manifest](../archive/.archive-manifest.json) — Сводная информация по архиву

## 📞 Support
- **Email**: support@albert3.app
- **Discord**: [Join Server](https://discord.gg/albert3)

## 📱 Mobile UI
- [Mobile UI Improvement Plan](./MOBILE_UI_IMPROVEMENT_PLAN.md) — План улучшений мобильного интерфейса
